...
<main role="main" class="flex-shrink-0">
    <div class="container">
        ...
        <div class="rounded border p-5">
            ...
            <h4 class="cover-heading">Edycja danych pojazdu</h4>
            <p>
            <form style="text-align: left" action="${pageContext.request.contextPath}/${cat}/vehicleEdit" method="post">
                ...
                <div class="form-group col-md-4">
                    ...
                    <select id="vehicleCustomerId" class="custom-select" aria-label="Imie i nazwisko klienta" name="vehicleCustomerId">
                        <c:forEach items="${customerList}" var="customer">
                            <option value="${customer.id}" <c:if test="${customerId!=null}"><c:if test="${customer.id == vehicle.customerId}">selected</c:if> <c:if test="${customer.id != vehicle.customerId}">disabled</c:if></c:if> >${customer.firstName} ${customer.lastName}</option>
                        </c:forEach>
                    </select>
